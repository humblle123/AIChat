...
class ReActAgent:
    ...
    def run(self, question: str):
        ...
        while current_step < self.max_steps:
            ...
            response = self.parse_react_output_json(response_text)
    ...
    def parse_react_output_json(self, text):
        try:
            parsed = json.loads(text)  # 假设LLM输出是纯JSON字符串
            if "Thought" not in parsed or "Action" not in parsed:
                return None
            return {"Thought": parsed["Thought"], "Action": parsed["Action"]}
        except json.JSONDecodeError:
            return None  # 格式错误时返回None，可触发重试
...
if __name__ == '__main__':
    ...
    question = "华为最新的手机是哪一款？它的主要卖点是什么？字数100字以内"
    agent.run(question)
